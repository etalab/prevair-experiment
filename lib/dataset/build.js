...
async function doStuff() {
  ...
  const date = `${periode.substr(0, 4)}-${periode.substr(4, 2)}-${periode.substr(6, 2)}`
  const result = {metadata: {date}, records: []}
  ...
  await bluebird.each(TYPES_POLLUANTS, async typePolluant => {
    return bluebird.each(TYPES_PREVISIONS, async typePrevision => {
      ...
      if (firstIteration) {
        ...
        Object.assign(result.metadata, {minLon, minLat, maxLon, maxLat})
      }
